...
private:
	...
	void TeleopPeriodic() {
		...
		if (GetFPGATime() > initialTime + 250) { //runs every quarter second
			SmartDashboard::PutNumber("Distance left", CommandBase::s_drivebase->GetLeftDistance());
			SmartDashboard::PutNumber("Distance right", CommandBase::s_drivebase->GetRightDistance());
			initialTime = GetFPGATime();
		}
